...
glm::vec3 basket[]{
    glm::vec3(-0.2, -0.9, 0.0),
    glm::vec3(0.2, -0.9, 0.0),
    glm::vec3(-0.2, -0.8, 0.0),
    glm::vec3(0.2, -0.8, 0.0),
};
float basket_x;
float basket_dx{0.01};
const unsigned int basket_index[]{
    0, 1, 3,
    0, 2, 3
};
...
GLvoid drawScene(GLvoid) {
    ...
    move_basket = glm::translate(move_basket, glm::vec3(basket_x, 0.0, 0.0));
    glUniform3fv(color, 1, glm::value_ptr(blue_color));

    glUniformMatrix4fv(trans_mat, 1, GL_FALSE, glm::value_ptr(move_basket));
    glBindVertexArray(VAO);
...
GLvoid init_buffer() {
    ...
    glGenBuffers(1, &VBO);
    glBindBuffer(GL_ARRAY_BUFFER, VBO);
    glBufferData(GL_ARRAY_BUFFER, sizeof(basket) * 12, basket, GL_DYNAMIC_DRAW);
    ...
    glGenBuffers(1, &EBO);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,  6 * sizeof(unsigned int), basket_index, GL_STATIC_DRAW);
